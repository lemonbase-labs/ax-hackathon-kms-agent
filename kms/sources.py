"""RSS-based article source + section-page article seeds for non-RSS sources.

Feed whitelist is loaded from kms/feeds.txt via kms._feeds.
"""
...
def apply_media_labels(candidates: list[dict]) -> list[dict]:
    ...
    domain_to_name.update({_normalize_domain(url): name for name, url in _feeds.load().items()})
